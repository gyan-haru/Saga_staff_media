...
この repo は、`コード本体` と `実データ` を分けて運用できる。
...
├── setup_runtime.py
├── import_transfers.py
├── import_transfer_directory.py
├── import_policy_sources.py
├── rebuild_person_mentions.py
├── rebuild_employee_slots.py
├── rebuild_policy_topics.py
├── templates/
├── static/
├── docs/
├── windows/
...
外部 runtime データ置き場を使う場合は、実DBやCSVは repo の外へ出せる。  
その場合のイメージは次のとおり。

```text
/path/to/saga_staff_media_data/
├── saga_media.db
├── crawled_urls.txt
├── list_sources.csv
├── department_hierarchy.csv
...
- `exports/`
- `transfers/`
- `transfers/source_index.csv`
- `transfers/transfer_template.csv`
- `policy_sources/`
- `policy_sources/policy_source_template.csv`
- `list_sources.csv`
- `department_hierarchy.csv`
...
既定では repo 内の `data/` を使う。  
Windows や別PCで repo とデータを分けたいときだけ、外部 runtime データ置き場を使う。

必要なら、既存DBやクロール済みログも一緒にコピーできる。

```bash
./.venv/bin/python setup_runtime.py --runtime-dir /path/to/saga_staff_media_data --copy-db --copy-transfers --copy-policy-sources
```

### 13-3. Windows での初回セットアップ
...
```bash
./.venv/bin/python import_policy_sources.py /path/to/policy_sources.csv
```
...
1. `policy_source_template.csv` 形式でCSVを作る
   既定では bundled の [data/policy_sources/policy_source_template.csv](/Users/harrn/Desktop/saga_staff_media/data/policy_sources/policy_source_template.csv) を参照できる。
   外部 runtime データ置き場を使う場合は `policy_sources/policy_source_template.csv` が初期化時に作られる。
